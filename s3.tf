...
resource "aws_s3_bucket" "ops_nfs_backup_bucket" {
  bucket = "${var.ops_nfs_backup_bucket}"
  acl    = "${var.ops_nfs_backup_acl}"
  region = "${var.region}"
  ...
  logging {
    target_bucket = "${var.log_archive_s3_bucket}"
    target_prefix = "ops_nfs_backup_bucket/"
  }

  tags = {
    Name = "ops_nfs_backup_bucket-${local.naming_suffix}"
  }
}

resource "aws_s3_bucket_policy" "ops_nfs_backup_bucket" {
  bucket = "${var.ops_nfs_backup_bucket}"
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "*",
      "Resource": "arn:aws:s3:::${var.ops_nfs_backup_bucket}/*",
      "Condition": {
        "Bool": {
          "aws:SecureTransport": "false"
        }
