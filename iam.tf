...
resource "aws_iam_role" "ops_win_freight" {
  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_role_policy" "ops_win_athena_freight" {
  name = "ops-win-athena-freight${local.naming_suffix}"
  role = "${aws_iam_role.ops_win_freight.name}"
...
resource "aws_iam_instance_profile" "ops_win_freight" {
  role = "${aws_iam_role.ops_win_freight.name}"
}
